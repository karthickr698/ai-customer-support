import { UnsafeNotificationUrlError } from './errors.js';

const BLOCKED_HOSTS = new Set(['localhost', 'metadata.google.internal']);
const LOCAL_HOSTS = new Set(['localhost', '127.0.0.1', '[::1]', '::1']);

export function assertSafeNotificationUrl(
  raw: string,
  label = 'URL',
  ...
  try {
    ...
    throw new UnsafeNotificationUrlError(`${label} must be a valid URL`);
  }

  if (parsed.username || parsed.password) {
    throw new UnsafeNotificationUrlError(`${label} must not include credentials`);
  }
  ...
  if (parsed.protocol !== 'https:' && !localHttpAllowed) {
    throw new UnsafeNotificationUrlError(`${label} must use https`);
  }

  if (!localHttpAllowed && (BLOCKED_HOSTS.has(host) || host.endsWith('.local') || host.endsWith('.internal'))) {
    throw new UnsafeNotificationUrlError(`${label} host is not allowed`);
  }
